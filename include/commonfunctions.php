...
function checkTableName($shortTName, $type=false)
{
	...
	if ("funcionario" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("nombramiento" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("contrato" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("perfil" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	...
	if ("contrato_forma_pago" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("contrato_natjuridica" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("contrato_otrosi_type" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	...
	if ("contrato_tipo_unidad" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("global_month" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	...
	if ("tipo_docidentidad" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("tipo_liquidacion" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	...
	if ("global_users" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("global_rol_almacen" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("global_rol_c" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	...
	if ("ep_agregada" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("q_graph_001_chart" == $shortTName && ($type===false || ($type!==false && $type == 3)))
		return true;
	if ("q_graph_002_chart" == $shortTName && ($type===false || ($type!==false && $type == 3)))
		return true;
	if ("q_graph_001" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("q_graph_002" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
...
function GetTablesList($pdfMode = false)
{
	...
	$tableAvailable = true;
	if( $checkPermissions ) {
		$strPerm = GetUserPermissions("contrato_otrosi_type");
		$tableAvailable = ( strpos($strPerm, "P") !== false
			|| $pdfMode && strpos($strPerm, "S") !== false );
	}
	if( $tableAvailable ) {
		$arr[]="contrato_otrosi_type";
	}
	$tableAvailable = true;
	if( $checkPermissions ) {
	...
	$tableAvailable = true;
	if( $checkPermissions ) {
		$strPerm = GetUserPermissions("q_graph_001");
		$tableAvailable = ( strpos($strPerm, "P") !== false
			|| $pdfMode && strpos($strPerm, "S") !== false );
	}
	if( $tableAvailable ) {
	...
	$tableAvailable = true;
	if( $checkPermissions ) {
		$strPerm = GetUserPermissions("q_graph_002");
		$tableAvailable = ( strpos($strPerm, "P") !== false
			|| $pdfMode && strpos($strPerm, "S") !== false );
	}
	if( $tableAvailable ) {
...
function ReadUserPermissions($userID = "")
{
	...
	if($userID != "Guest")
	{
		...
		$sql = "select ".$gConn->addFieldWrappers( "GroupID" )
			...
			." from ". $gConn->addTableWrappers( "gestionglobal_ver_103_ugmembers" )
	...
	$sql = "select ". $gConn->addFieldWrappers( "TableName" )
		...
		." from ". $gConn->addTableWrappers( "gestionglobal_ver_103_ugrights" )
...
//	default permissions
	...
	if( $table=="q_graph_001" )
	{
//	default permissions
		// grant all by default
		return "ADESPI".$extraPerm;
	}
	if( $table=="q_graph_002" )
	{
//	default permissions
		// grant all by default
		return "ADESPI".$extraPerm;
